    ...
    if (chrome.topSites && chrome.topSites.get) {
      chrome.topSites.get((sites) => {
        const topFiveSites = sites.slice(0, 5).map((site) => ({
          title: site.title,
          url: site.url,
          favIconUrl: `chrome-extension://${chrome.runtime.id}/_favicon/?pageUrl=${encodeURIComponent(site.url)}&size=38` || './logo-white.png',
        }));
          ...
          {topSites.length === 0 ? (
            ...
            topSites.map((site, index) => (
              ...
              >
                <div className="mx-auto p-2.5 w-14 h-14 bg-slate-300 hover:bg-accent text-center rounded-md items-center">
                  <img
                    src={site.favIconUrl || './logo-white.png'}
                    alt={site.title}
                  />
                </div>
